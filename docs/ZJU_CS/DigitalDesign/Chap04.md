...
### JK 触发器
JK触发器的状态转换规则如下：
- 当J=1，K=0时，触发器将置位（Q=1）；
- 当J=0，K=1时，触发器将复位（Q=0）；
- 当J=1，K=1时，触发器将切换状态（如果之前是置位状态，则变为复位状态；如果之前是复位状态，则变为置位状态）；
- 当J=0，K=0时，触发器保持原状态不变

### T 触发器
$Q_{n+1} = \overline{Q_n}T + Q_n\overline{T} = Q_n \oplus T$
...
**Hidden Table**(是考点) 建议去**看看PPT**，这里就不写了。
...
<!-- prettier-ignore-start -->
??? info "Time"
    t_{pd}（传播延迟时间）：这是指输入发生变化后，输出发生变化所需的时间。它表示了从输入发生变化到输出发生变化之间的延迟时间。
    t_{s}（Setup Time）：这是指在时钟信号作用下，使得触发器的输出从低电平变为高电平所需的时间。在这段时间内，设置信号必须保持稳定，以确保触发器正确地将输出置为高电平。
    t_{h}（Hold Time）：这是指在时钟信号作用下，使得触发器的输出从高电平变为低电平所需的时间。在这段时间内，保持信号必须保持稳定，以确保触发器正确地将输出置为低电平。
<!-- prettier-ignore-end -->
