...
1. 确定系统的行为；
2. 阐述输入和输出之间的逻辑关系，并用真值表或逻辑表达式(Formulation)表达出来；
3. 优化逻辑表达以减少成本(比如使用卡诺图)；
4. 将优化后的逻辑设计工艺映射到硬件实现上(Technology Mapping);
5. 验证正确性（在仿真环境中）；
...
!!! note "知识点"
    1. ⼀个函数的具体实现称为功能块.
    2. SSI MSI LSI ⼩中⼤规模集成电路
    3. VLSI 超⼤规模集成电路
<!-- prettier-ignore-end -->
---

### 基本功能块
> 这里我们更推荐通过瓜豪学长的[实验文档](https://guahao31.github.io/2023_DD/)来学习。

- 译码器(Decoder)
- 编码器(Encoder)

- （三端）多路复用器(Multiplexer) `MUX`（类似于旋转开关，但实际上我们可以通过电路图实现，具体可以看实验课）
...
更多的内容可以看**[ xyx 的计组笔记](https://xuan-insr.github.io/computer_organization/3_arithmetic/#311-1-bit-alu)**。（数逻中最多了解到超前进位加法器即可。）
